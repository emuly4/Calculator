...
static const uint8_t RB_MASK_C = RB_LENGTH_C - 1;
...
void rb_push_back_C(struct Ring_Buffer_C *p_buf, char value)
{
    ...
    p_buf->buffer[p_buf->end_index] = value;
    // Increment the end index and wrap using the mask.
    uint8_t new_end_index = (p_buf->end_index + 1) & RB_MASK_C;
    p_buf->end_index = new_end_index;
    // If the end equals the start increment the start index
    if (p_buf->start_index == p_buf->end_index)
    {
        uint8_t new_start_index = (p_buf->start_index + 1) & RB_MASK_C;
        p_buf->start_index = new_start_index;
    }
}
...
float rb_get_F(const struct Ring_Buffer_F *p_buf, uint8_t index)
{
    ...
    uint8_t new_index = (p_buf->start_index + index) & RB_MASK_F;
    return p_buf->buffer[new_index];
    ;
}
char rb_get_C(const struct Ring_Buffer_C *p_buf, uint8_t index)
{
    // return value at start + index wrapped properly
    uint8_t new_index = (p_buf->start_index + index) & RB_MASK_C;
    char value = p_buf->buffer[new_index];
    // your code here!
    return value;
}
